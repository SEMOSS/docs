...
For instructions on how to perform a frontend installation of SEMOSSName instead, click  **[here](Frontend%20Installation.md)**.

> **Note**
> If you don't need admin privileges and would prefer a **lightweight, low management** way to connect to SEMOSSName, then use the [live web server](https://semoss.com/demo/SemossWeb/packages/client/dist/) instead of following this guide. Instructions to set up a connection to the web version can be found in the [Connecting to SEMOSSName](../Integrating%20with%20SEMOSS/ConnectingToAI.md) guide.

## Before You Begin

- **External Docker/container deployments are not supported on SEMOSSName.**
- Custom container deployments require full  cyber, and risk approvals, which are not supported for standard app onboarding.
- Applications intended for production use must be deployed in the user’s own cloud environment and accessed via API.
- Backend extensions on SEMOSSName should be implemented using supported technologies (e.g., Java or Python).

This guide documents how to run SEMOSSName inside a Docker container for **local development and internal usage scenarios only**.

Docker containers created using this guide **must not be considered a supported production deployment model** for SEMOSSName-hosted applications.
...
## What's Next?
Want to start developing apps with SEMOSSName? Learn how to use the SEMOSSName Software Development Kit (SEMOSS SDK) in the [SEMOSS SDK Guide](../Integrating%20with%20SEMOSS/Using%20the%20SDK.md). 
