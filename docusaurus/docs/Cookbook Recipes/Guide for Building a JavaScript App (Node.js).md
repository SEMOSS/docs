---
title: Guide for Building a Javascript App (Node.js)
description: Build, run, and deploy a GenAI application using React, TypeScript, and the SEMOSS platform, including local development and publishing to the App Catalog.
...
Node.js is supported for:
- Frontend development
- Local development servers
- Build tooling (webpack, pnpm, etc.)
- Packaging and bundling applications

Node.js is **not supported as a backend runtime within SEMOSSName**.

SEMOSSName does not support deploying Node.js server applications or custom containerized Node backends. Backend logic must be implemented using supported platform technologies (e.g., Java or Python) and deployed according to SEMOSSName architectural guidelines.

If your application requires a custom Node.js backend service, it must be deployed in your own cloud environment and accessed via API.
...
You can see app examples in the [SEMOSSName Apps](https://github.com/SEMOSS/training) repo. Feel free to download an existing app and use it as scaffold for building your own! Existing work can be used to experiment and learn.
> **Note**
> SEMOSSName Apps is a protected GitHub, if you do not have access to it, or are not currently signed in to your enterprise GitHub account you will get a 404 Error. Ask an admin for access.
<!--Should we link to the public   demo here so that first time users can click around and explore the apps in the live site? For a user encountering this guide for the first time, they probably won't know how to interact with the apps in the  ai-apps repo. It might be useful to copy the "Adding your app to the SEMOSSName catalog" section of this guide over to the  ai-apps repo's README.md so that users can be guided on what to do after they've pulled the repo (ex. zipping the files and adding the app within  )-->
<!--Since this is a React app use case, do all the examples in the  ai-apps repo also use React? I'm not sure how important this is but it might be helpful for users following this guide to base their app off of another React app in the  ai-apps repo-->

## App Structure

Below is the typical structure of a React app to be uploaded onto the SEMOSSName platform. The `/java` and `/py` directories hold back-end work. This document will focus on the `client` and `portals` folders.
<pre>
  <code>
...
See the Environment Variables section of [Using the SDK](../Integrating%20with%20SEMOSS/Using%20the%20SDK.md#environment-variables)
...
## Connecting to a SEMOSSName Instance
See [Connecting to SEMOSSName](../Integrating%20with%20SEMOSS/ConnectingToAI.md)
for documentation on how to set up your app with API keys to be added as environment variables.

## Using the SEMOSS SDK

Learn more about how to use the SEMOSS SDK in the [Using the SDK](../Integrating%20with%20SEMOSS/Using%20the%20SDK.md) guide.
...
```shell
PS C:\Development\apps\llm-tuner\client> npm run dev

> dev
> webpack serve

<i> [webpack-dev-server] [HPM] Proxy created: /Monolith  -> MonoLithURL
<i> [webpack-dev-server] Project is running at:
<i> [webpack-dev-server] Loopback: http://localhost:3000/, http://127.0.0.1:3000/
<i> [webpack-dev-server] Content not from webpack is served from 'C:\Development\apps\llm-tuner\client\public' directory
...
# Creating a SEMOSSName GenAI App Using React
...
2. Navigate to [App Library](https://MonoLithURL/SemossWeb/packages/client/dist/) and click "Create a New App" in the top right corner. This will bring you to the Create New App page.
...
6. Ta-da! You have successfully uploaded your code app to the SEMOSSName Server!

## Updating Your App in SEMOSS

Once you have finished making any updates to your app, these are the steps that you should follow to update your app in SEMOSS.
