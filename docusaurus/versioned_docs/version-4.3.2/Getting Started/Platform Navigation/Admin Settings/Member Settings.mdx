---
sidebar_label: "Member Settings"
sidebar_position: 1
slug: "/platform-navigation/AdminSettings/MemberSettings"
---

### Add Members
1. Click on Settings
2. Enable Admin Mode
![Enable Admin Mode](/versioned/4.3.2/autoImg/PlatformNavigation/Member%20Settings/Admin%20On.png)
3. Click on Member Settings
4. Click on Add Member
5. Fill in user details
![Admin Details](/versioned/4.3.2/img/AdminSettings/AddAdminDetails.png)
In the Permissions Section you can grant Admin, Publisher, and Exporter access
6. You can also add Model Limit Restrictions for the new user
![Model Limits](/versioned/4.3.2/img/AdminSettings/ModelLimit.png)
Admins can limit model limits by token or compute time
7. Click on Save

### Edit Members
![EditMembers](/versioned/4.3.2/autoImg/PlatformNavigation/Member%20Settings/MemberSettingsPage.png)
This view displays a list of all existing members, along with their assigned roles (e.g., Publisher, Exporter, Admin).
    - Use the checkboxes or the pencil icon to edit member permissions.
    - Use the trashcan icon to remove members from the platform.





