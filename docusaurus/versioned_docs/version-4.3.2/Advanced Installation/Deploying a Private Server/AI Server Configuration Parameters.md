...
- name: MS_REDIRECT
    value: >-
    https://MonoLithURL/Monolith/api/auth/login/ms
- name: MS_SECRET_KEY
    value: secret_key
- name: MS_TENANT
    value: semoss.onmicrosoft.com
````
...
9. **REDIRECT:**
   ...
   * Example Value: https://MonoLithURL/SemossWeb/
...
12. **MS_REDIRECT:**
   ...
   * Example Value: https://MonoLithURL/Monolith/api/auth/login/ms
...
14. **MONOLITH_ROUTE:**
   * Description: This is an optional environment variable that depends on the existence of a subpath called demo/monolith. If you don't have this subpath, you can safely ignore this variable.
   * Value: demo

15. **FE_ROUTE:**
   * Description: This will come into existence if a subpath called demo/monolith exists, and it will serve as your monolith endpoint.
   * Value: /demo/Monolith
...
spec:
  ports:
    - protocol: TCP
      ...
      targetPort: 8080
  selector:
    app.kubernetes.io/instance: semoss-ai
    app.kubernetes.io/name: semoss-ai
  type: ClusterIP
...
In the given YAML configuration for a Kubernetes Service, the spec field defines the desired state of the Service.
   ...
   * selector field is used to select the Pods that the Service will route traffic to. In this case, the Service selects Pods with the labels app.kubernetes.io/instance: semoss-ai and app.kubernetes.io/name: semoss-ai.
...
spec:
  ...
  tls:
    - hosts:
        - YOUR_DEPLOYMENT_DOMAIN
      secretName: workshop-tls
  rules:
    - host: YOUR_DEPLOYMENT_DOMAIN
      http:
        paths:
          - path: /
...
**Example 2:** Ingress with sub-Path /semoss-ai/(.*)

This configuration establishes an ingress rule for the sub-path /semoss-ai/(.*) directing traffic to a different service with a specific port. Adjust the service names, ports, and domain according to your specific setup. It also includes a rewrite annotation for handling the sub-path.
````
...
In the given YAML configuration for an Ingress resource in Kubernetes, the spec field defines the desired state of the Ingress.
   ...
   * tls section defines the TLS configuration for the Ingress. In this case, it specifies that the host YOUR_DEPLOYMENT_DOMAIN should be secured using the TLS certificate stored in the Secret named workshop-tls.
   * rules section defines the routing rules for the Ingress. In this example, there is a single rule for the host YOUR_DEPLOYMENT_DOMAIN. Requests matching the specified path /semoss-ai/(.*) will be forwarded to the backend service named semoss-ai on port 8080.
   ...
   * This configuration sets up an Ingress resource that uses the nginx Ingress controller, secures the YOUR_DEPLOYMENT_DOMAIN host with TLS using the workshop-tls Secret, and forwards requests to the semoss-ai service on port 8080 for the specified path.
