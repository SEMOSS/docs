...
The Java source file for `MyTodoReactor` can be found [here](/assets/CustomReactorAppZips/MyTodoReactor.java).

To add the `MyTodoReactor` to the app context:
1. Navigate to the Files tab. If it does not exist yet, create a `java` folder inside of the `version/assets` folder. Within the `java` folder, create the subfolders for your reactor based on its package, then upload the reactor file: ![Folder tree](/img/CustomReactor/FolderTree.jpg)
...
2. Save the app, then return to the SEMOSSName homepage. Find your app ID by clicking on the three dots icon on the lower right hand corner of the app, then click "Copy App ID" and save the copied ID to a safe place. ![App ID](/img/CustomReactor/AppID.jpg)
...
![App ID](/img/CustomReactor/AppSettings.jpg)
...
```
![Notebook](/img/CustomReactor/MyTodoNotebook.jpg)
...
![Task 1](/img/CustomReactor/Task1.jpg)
Output: ![Task 2 Output](/img/CustomReactor/Task1Output.jpg)

2. Submit a form where the task ID = 3 and the task name is changed to "Say goodbye":
![Task 3](/img/CustomReactor/Task3.jpg)
Output: ![Task 3 Output](/img/CustomReactor/Task3Output.jpg)

## Conclusion and Additional Links
Custom reactors are powerful because they allow you to integrate a wide array of Java libraries into the app and extend the functionality of your app beyond the native SEMOSSName reactors. 
