...
The SEMOSS SDK (Software Developer Kit) gives you access to the data that you have stored inside of your SEMOSSName instance. It also allows you to make pixel calls, login, and query LLMs. 

## Using the SEMOSS SDK
In order to use the SEMOSS SDK, you will need to populate your SEMOSSName instance with models, storage catalogs, and databases for use in your project. Learn how to do that in [Navigating SEMOSSName](../../Get%20Started/Navigation).

## Environment Variables
The following environment variables are required at the root level of your client directory. They should be defined in `.env` and `.env.local` files. `.env.local` will not already exist in the project and will need to be created. Set the `ACCESS_KEY` and `SECRET_KEY` variables in `.env.local` to be the [access and secret keys](./ConnectingToAI.md#generating-access-and-secret-keys) you generated to connect to SEMOSSName.

![Screenshot of .env and .env.local file contents](/img/FELocalInstall/env.png)
...
The example `.env` above demonstrates how you would set your environment variables to connect to the SEMOSSName server. To connect to a local instance instead, use the following variables:
```
    MODULE=/Monolith_Dev
    ENDPOINT=http://localhost:9090
    APP=<APP_ID>
```
You can access the `APP_ID` after you host the app in SEMOSSName. Your `APP_ID` will be an alphanumerical string at the end of the app URL. 
...
## Using Pixels
you can access it by using `pixel` API calls. Pixel is a domain-specific language used by SEMOSSName to access and update data. Below are some examples of how to use a pixel call.
...
SEMOSSName Platform provides 2 main ways to query a database.
- Generic Pixel Query syntax: This is SEMOSSName specific querying syntax which is database agnostic.
...
```
> _**Note** For more guidance on the syntax of LLM and database queries, please review the SEMOSSName User Guide **[here](https://semoss.org/SemossDocumentation/#scripting-understanding-pixel)**._
